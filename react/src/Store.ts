...
export default class Store {
  public static readonly DB_NAME: string = 'React-TODO';
   ...
   * 文字列から新たな Todo を追加する
   */
  public async addTodo(text: string): Promise<ITodo[]> {
    if (!this.db) {
      await this.openDB();
    }
    await this.addTodoObject(text);
    const list: ITodo[] = await this.getAllTodoObject();
    return list;
  }
   ...
   * 全ての Todo を取得する
   */
  public async getTodoAll(): Promise<ITodo[]> {
    if (!this.db) {
      await this.openDB();
    }
    const list: ITodo[] = await this.getAllTodoObject();
   ...
   * 完了済み Todo を全て削除する
   */
  public async clearCompleted(): Promise<ITodo[]> {
    if (!this.db) {
      await this.openDB();
    }
  ...
  protected getAllTodoObject(): Promise<ITodo[]> {
  ...
  protected addTodoObject(text: string): Promise<ITodo> {
    return new Promise<ITodo>((resolve, reject) => {
      const data = {
        isCompleted: false,
        text,
  ...
  protected updateTodoObject(todo: ITodo): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      const store = this.getObjectStore(Store.DB_STORE_NAME, 'readwrite');
  ...
  protected async clearCompletedTodoObject(): Promise<void> {
    const delTaskList = await this.getAllTodoObject().then((list: ITodo[]): IPromiseArray<void> => {
      return list.reduce(
        (delTasks: IPromiseArray<void>, todo: ITodo): IPromiseArray<void> => {
          if (todo.isCompleted) {
